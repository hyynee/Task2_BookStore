...
import { createCheckout } from '../../redux/Reducer/checkoutReducer';
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto py-10 px-6 tracking-tighter'>
                    ...
                    <div className='mt-6'>
                        {!checkoutId ? (
                            ...
                            <div className=''>
                                ...
                                {/* paypal component*/}
                                <Paypal amount={cart.totalPrice} onSuccess={handlePaymentSuccess} onError={(err) => alert("payment fail. Try again!")} />
                            </div>
                        )}
                    </div>
                </form>
            ...
            <div className='bg-gray-50 p-6 rounded-lg'>
                ...
                <div className='border-t py-4 mb-4'>
                    {cart.items.map((book, index) => {
                        return <div key={index} className='flex items-start justify-between py-2 border-b'>
                            <div className='flex items-start'>
                                <img src={book.image} alt={book.title} className='w-16 h-20 object-cover mr-4' />
                                <div>
                                    <h3 className='text-md'>{book.title}</h3>
                                    <p className='text-gray-700'>Quantity: {book.quantity}</p>
                                    <p className='text-gray-700'>Price: ${book.price?.toLocaleString()}</p>
                                </div>
                            </div>
                            <p className='text-xl'>$ {(book.price * book.quantity)?.toLocaleString()}</p>
                        </div>
                    })}
                </div>
